...
if ($type == "get_listing_data") {
    $columns = array(
        ...
        array('db' => 'title', 'dt' => 1,'formatter'=>function($d,$row){
            ...
            if($row['parent_id']!=0){
                $query="select * from ".$table['module']." where pkid=".$row['parent_id'];
                $result=$database->query($query);
                $rs_array=$result->fetchRow();

                return $rs_array['title']." <i class='fa fa-angle-double-right'></i> ".$d;
            }else{
                return $d;
            }
        }),
    ...
    $where="";
    echo json_encode(
        SSP::complex($_GET, $sql_details, $table['module'], 'pkid', $columns, null, $where)
    );
    exit();
}
...
if ($_POST['method'] == "delete_listing") {
    $pkid = $_POST['pkid'];
    $query=get_query_delete($table['module'],$pkid);
    $database->query($query);

    do_tracking($user_username, 'Delete Module');
